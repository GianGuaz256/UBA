...
use uba::{
    default_public_relays, derive_encryption_key, generate_random_key, AddressGenerator, UbaConfig,
    UbaEncryption,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Generated {} addresses across all layers",
        addresses1.get_all_addresses().len()
    );

    // Count addresses by type
    let btc_l1_count = addresses1
        .get_addresses(&uba::AddressType::P2PKH)
        .map(|v| v.len())
        .unwrap_or(0)
        + addresses1
            .get_addresses(&uba::AddressType::P2SH)
            .map(|v| v.len())
            .unwrap_or(0)
        + addresses1
            .get_addresses(&uba::AddressType::P2WPKH)
            .map(|v| v.len())
            .unwrap_or(0)
        + addresses1
            .get_addresses(&uba::AddressType::P2TR)
            .map(|v| v.len())
            .unwrap_or(0);
    let liquid_count = addresses1
        .get_addresses(&uba::AddressType::Liquid)
        .map(|v| v.len())
        .unwrap_or(0);
    let lightning_count = addresses1
        .get_addresses(&uba::AddressType::Lightning)
        .map(|v| v.len())
        .unwrap_or(0);
    ...
    println!(
        "Encryption key hex: {}",
        config.get_encryption_key_hex().unwrap()
    );
    ...
    println!(
        "Generated {} addresses with encryption config\n",
    ...
    println!(
        "After adding relay: {:?}",
        config_with_relays.get_relay_urls()
    );
    ...
    println!(
        "P2WPKH count: {}",
        config_counts.get_address_count(&uba::AddressType::P2WPKH)
    );
    println!(
        "Liquid count: {}",
        config_counts.get_address_count(&uba::AddressType::Liquid)
    );
    println!(
        "Lightning count: {}",
        config_counts.get_address_count(&uba::AddressType::Lightning)
    );

    let generator_counts = AddressGenerator::new(config_counts);
    let addresses_counts =
        generator_counts.generate_addresses(seed, Some("custom-counts".to_string()))?;
    println!(
        "Total addresses with custom counts: {}",
        addresses_counts.get_all_addresses().len()
    );
