...
fn display_addresses_by_type(addresses: &uba::BitcoinAddresses, addr_type: &AddressType, type_name: &str) {
...
fn show_uba_construction_example(nostr_id: &str) {
    ...
    println!("   2. Create UBA with label:    UBA:{}&label=my-wallet", nostr_id);
    println!("   3. Use uba::retrieve() or uba::retrieve_full() to get addresses\n");
    
    println!("📝 EXAMPLE CODE:");
    println!(r#"   let uba = "UBA:{}";
   let addresses = uba::retrieve(uba, &relay_urls).await?;"#, nostr_id);
}
...
fn show_retrieval_workflow_example() {
    ...
    println!(r#"   let parsed = uba::parse_uba("UBA:abc123...&label=wallet")?;
