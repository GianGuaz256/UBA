...
use uba::{
    ...
    derive_encryption_key, generate_random_key, default_public_relays
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Encryption key (hex): {}", config2.get_encryption_key_hex().unwrap());
    ...
    println!("Retrieved {} addresses from encrypted data\n", addresses2.len());
    ...
    // Show default relays
    println!("Default public relays ({} total):", default_public_relays().len());
    ...
    println!("\nAfter resetting to defaults: {} relays", config4.get_relay_urls().len());
    ...
    match retrieve_with_config(&uba2, &[], wrong_config).await {
        Ok(addresses) => println!("Retrieved {} addresses (fallback to unencrypted)", addresses.len()),
